 ...
 * @return @ref t_eReturnCode
 */
static t_eReturnCode s_HC_HardAxeStop(t_eHC_AxeHandleList f_idxAxe_e);
/**
...
/**
 * @brief Compute the actual position {x,y} from angle alpha_b (angle between xa & xb see drawIO)
 *          and alpha_c (angle between xb & xc)
 * 
 * ----------------------------------------------------------------------------
 * @param[in] f_alphaB_f32 : Angle position of joint Cntr Knive and support head
 * @param[in] f_alphaC_f32 : Angle position of joint Knive & CntrKnive
 * @param[in] f_posX_f32 : Position X in ref A 
 * @param[in] f_posY_f32 : Position Y in ref A 
 * ----------------------------------------------------------------------------
 * @return @ref t_eReturnCode
 */
...
/**
 * @brief Compute the actual position the angle position of each joint 
 *          based on ref A position {x,y}
 * 
 * ----------------------------------------------------------------------------
 * @param[in] f_posY_f32 : Position Y in ref A 
 * @param[in] f_posX_f32 : Position X in ref A 
 * @param[in] f_alphaB_pf32 : Angle position of joint Cntr Knive and support head
 * @param[in] f_alphaC_pf32 : Angle position of joint Knive & CntrKnive
 * ----------------------------------------------------------------------------
 * @return @ref t_eReturnCode
 */
...
t_eReturnCode HEAD_CUTTER_Init(void)
{
    ...
    if(Ret_e == RC_OK)
    {
        ...
        g_calibCmdInfo_s.reqSts_e = APPLGC_CALIB_STS_IDLE;
        g_calibCmdInfo_s.currSts_e = APPLGC_CALIB_STS_IDLE;
...
static t_eReturnCode s_HC_Fsm_PrdTsk_Operational(void)
{
    ...
    if(g_calibCmdInfo_s.reqSts_e == APPLGC_CALIB_STS_MOVE)
    {
...
static t_eReturnCode s_HC_UpdatePosition(void)
{
    ...
    Ret_e = APPSYS_GetSysOption(APPSYS_OPT_ID_SNS_ECDR_HD_CNTR_KNF, &sysOptKnv_u8);
    ...
    else 
    {
        ...
        switch(f_msgID_u16)
        {
            ...
            case APPSIG_CAN_MSG_LGC_HC_CMD_CALIBRATION:
            {
                ...
                {
                    sigSnsID_e = (t_eAPPSIG_Signal)(f_sigValue_af32[0]);
                    ...
                    || (sigSnsID_e == APPSNS_SNSITF_ECDR_HD_HOLD_POS))
                    {
                        ...
                        if(Ret_e == RC_OK)
                        {
                            ...
                            else 
                            {
                                ...
                                calibCmdInfo_s.speed_f32 = cmdSpeed_f32;
            ...
            case APPSIG_CAN_MSG_LGC_REARMAMENT_CMD:
            {
                ...
                {
                    agent_e = f_sigValue_af32[0];
                    rearmType_e = f_sigValue_af32[1];
