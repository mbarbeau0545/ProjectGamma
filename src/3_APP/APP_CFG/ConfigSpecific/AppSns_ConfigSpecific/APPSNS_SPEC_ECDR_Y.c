...
t_eReturnCode APPSNS_SPEC_ECDR_Y_SetCfg(t_uint8 f_snsDvcOpt_u8, t_eAPPSNS_SnsDriverList *f_drvUsed_pe)
{
    ...
    else 
    {
        ...
        {
            case APPSYS_OPT_SNS_ECDR_Y_UNUSED:
                *f_drvUsed_pe = APPSNS_DRV_NB;
                Ret_e = RC_WARNING_NO_OPERATION;
                break;

            case APPSYS_OPT_SNS_ECDR_Y_NEMA17:
            {
            ...
            case APPSYS_OPT_SNS_ECDR_Y_APPSIG:
                *f_drvUsed_pe = APPSNS_DRV_NB;
                Ret_e = RC_OK;
            break;
            case APPSYS_OPT_SNS_ECDR_Y_NB:
            default:
...
t_eReturnCode APPSNS_SPEC_ECDR_Y_POS_GetSigValue(t_float32 *f_rawSigValue_pf32, t_bool * f_isValue_OK)
{
    ...
    else 
    {
        switch (g_OptSysCfg_u8)
        {
            ...
            case APPSYS_OPT_SNS_ECDR_Y_APPSIG:
                Ret_e = APPSIG_GetSignalValue(  APPSIG_SIGNAL_SNS_ECDR_Y_POS,
                                                &position_f32);
                if(Ret_e == RC_OK)
                {
...
t_eReturnCode APPSNS_SPEC_ECDR_Y_SPD_GetSigValue(t_float32 *f_rawSigValue_pf32, t_bool * f_isValue_OK)
{
    ...
    else 
    {
        switch (g_OptSysCfg_u8)
        {
            ...
            case APPSYS_OPT_SNS_ECDR_Y_APPSIG:
                Ret_e = APPSIG_GetSignalValue(  APPSIG_SIGNAL_SNS_ECDR_Y_SPD,
                                                &speed_f32);
                if(Ret_e == RC_OK)
                {
